...
// Sql
const WhatsSiriDB = config.DATABASE.define('WhatsSiri', {
    info: {
...
async function WhatsSiri () {
    ...
    conn.on ('credentials-updated', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await WhatsSiriDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                if (error.message.includes('URL')) {
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [WHATSIRI] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
